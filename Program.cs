        ...
        private static void Main(string[] args)
        {
            ...
            Parallel.For(0, CyclesToRun, i => // 400% performance increase
            {
                try
                {
                    ...
                    FileLogger.LogMessage($"Cycle {i} Finished. Total Cycle Time: {cycleElapsedTime.TotalMilliseconds:N} ms.");
                }
                catch (Exception ex)
                {
            ...
            totalMonitor.Stop();
            FileLogger.LogMessage("-----");
            FileLogger.LogMessage($"Execution Finished. Total Elapsed Time: {totalMonitor.Elapsed.TotalMilliseconds:N} ms.");
            Console.Read();
        }
    }
}